...
```bash
pip install rapidfuzz httpx sqlalchemy psycopg2-binary python-dotenv
```
...
# Verify
python -c "from app.db import get_db; from app.models import ShelfLifeReference; db = next(get_db()); print(db.query(ShelfLifeReference).count(), 'rows')"
```

### 11.4 Runtime estimate
